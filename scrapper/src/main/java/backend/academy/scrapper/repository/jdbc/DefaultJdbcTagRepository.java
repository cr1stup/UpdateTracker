...
public class DefaultJdbcTagRepository implements JdbcTagRepository {
    ...
    public void add(Long chatLinkId, List<String> tags) {
        ...
        tagIds.forEach(tagId -> client.sql(insertChatLinkTagSql)
    ...
    public void remove(String tag) {
        client.sql("DELETE FROM tag WHERE name = :tag").param("tag", tag).update();
    }
    ...
    public List<Long> findByName(String tag) {
        ...
        return client.sql(sql).param("tag", tag).query(Long.class).list();
    }
}
