...
public class DefaultJdbcLinkRepository implements JdbcLinkRepository {
    ...
    public List<Link> findLinksCheckedAfter(Duration after, int limit) {
        ...
        return client.sql(sql)
                .param(
                        "last_checked_at",
                        OffsetDateTime.now(ZoneId.systemDefault()).minus(after))
                .param("limit", limit)
                .query(Link.class)
                .list();
    }
    ...
    public void update(long id, OffsetDateTime lastModified) {
        client.sql(
                ...
                WHERE id = :id""")
                .param("last_checked_at", OffsetDateTime.now(ZoneId.systemDefault()))
    ...
    public void remove(long linkId) {
        client.sql("DELETE FROM link WHERE id = :id").param("id", linkId).update();
    }
}
