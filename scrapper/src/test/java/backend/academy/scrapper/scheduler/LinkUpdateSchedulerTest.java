...
class LinkUpdateSchedulerTest {
    ...
    private static final Link LINK_1 = new Link(1L, "https://github.com/example/repo1",
        List.of("tag1"), List.of("filter1"), OffsetDateTime.now(), OffsetDateTime.now().minusMinutes(20));
    private static final Link LINK_2 = new Link(2L, "https://github.com/example/repo2",
        List.of("tag2"), List.of("filter2"), OffsetDateTime.now(), OffsetDateTime.now().minusMinutes(40));
    ...
    private void verifyBotClientUpdates(Link link, List<Long> chatIds) {
        verify(botClient).handleUpdates(argThat(update ->
    ...
    private void mockClientFactory() {
        ...
        LinkInformation linkInfo1 = new LinkInformation(URI.create(LINK_1.url()), "Repo1", "Description1", OffsetDateTime.now());
        LinkInformation linkInfo2 = new LinkInformation(URI.create(LINK_2.url()), "Repo2", "Description2", OffsetDateTime.now());
